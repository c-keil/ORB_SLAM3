...
namespace ORB_SLAM3
{
    ...
    const double ORBmatcher::TH_HIGH = 0.7; // 100;
    const double ORBmatcher::TH_LOW = 0.5;  // 50;
        ...
        while(KFit != KFend && Fit != Fend)
        {
            if(KFit->first == Fit->first)
            {
                ...
                for(size_t iKF=0; iKF<vIndicesKF.size(); iKF++)
                {
                    ...
                    double bestDist1=256;
                    int bestIdxF =-1 ;
                    double bestDist2=256;

                    double bestDist1R=256;
                    int bestIdxFR =-1 ;
                    double bestDist2R=256;
...
// http://graphics.stanford.edu/~seander/bithacks.html#CountBitsSetParallel
    double ORBmatcher::DescriptorDistance(const cv::Mat &a, const cv::Mat &b)
    {
